...
class OvertopBassanoV16Production:
    ...
    def __init__(self, heartbeat_data=None, db_execute=None, heartbeat_lock=None):
        ...
        # -- Carica storia trade dal DB al boot (per OracleAuto) -----------
    ...
    def _open_trade(self, price, momentum, volatility, trend, regime,
        ...
        self._trade_peak_pnl       = 0.0    # massimo PnL raggiunto intra-trade
        self._trade_peak_ts        = None   # timestamp del picco
        self._trade_peak_energia   = 0.0    # exit_energy al momento del picco
    ...
    def _evaluate_exit(self, price, momentum, volatility, trend):
        ...
        duration_avg     = self._trade.get('duration_avg', 60.0)
        ...
        # ── TRACKING PICCO INTRA-TRADE ──────────────────────────────────
        _pnl_live = pnl_lordo - self.FEE_TRADE
        ...
        # ── CAPSULE MIGLIORA — correzioni exit calcolate una volta sola ─
        _exit_adj = self.capsule_memory.get_exit_adjustments(
            entry_direction,
            self._trade.get('regime', self._regime_current),
        ...
        profit_comp = 15 if max_profit <= 0 else max(5, int(
            (1.0 - min(1.0, retreat / max(max_profit, 0.01))) * 25))
        exit_energy = mom_score + trend_score + decel_comp + profit_comp

        # Aggiorna energia al picco — sincronizzata con l'aggiornamento del picco PnL
        if self._trade_peak_ts is not None and _pnl_live >= self._trade_peak_pnl:
            self._trade_peak_energia = exit_energy

        # Soglia uscita adattiva — corretta da capsule MIGLIORA
        exit_soglia = max(20, max(30, 60 - int(duration / 10) * 2) + _exit_adj['exit_soglia_delta'])
        ...
        if duration > duration_avg * 5 and drawdown_pct > 1.0:
            self._close_trade(price, "TIMEOUT_DD", momentum, volatility, trend)
        elif duration > 300:
    ...
    def _close_trade(self, price, reason, momentum, volatility, trend):
        ...
        score            = self._trade.get('score', 0)
        soglia           = self._trade.get('soglia', 0)
        ...
        # ── ORACOLO 2.0 — impara con contesto completo ──────────────────
        _rp = 0.5
        _dr = 0.0
        ...
        self.oracolo.record(
            entry_momentum, entry_volatility, entry_trend,
            is_win,
            direction=direction,
            pnl=pnl_netto,
            duration=duration,
        ...
        self.calibratore.registra_osservazione(
            seed_score=score / 100.0,
            fingerprint_wr=self._trade_entry_fingerprint or 0.72,
            is_win=is_win,
            divorce_drawdown_usato=(
                (self._trade_max_price - price) / entry_price * 100
            ) if self._trade_max_price else 0.0
        )

        # ── LOG ANALYZER ────────────────────────────────────────────────
        self.log_analyzer.registra({
            'matrimonio': matrimonio_name, 'pnl': pnl_netto, 'is_win': is_win
        })

        # ── CAPSULE MEMORY: EVITA/MIGLIORA ───────────────────────────────
        trade_data = {
            'pnl': pnl_netto, 'pnl_lordo': pnl_lordo, 'reason': reason,
            'direction': direction, 'regime': regime,
            'momentum': entry_momentum, 'volatility': entry_volatility,
            'trend': entry_trend, 'matrimonio': matrimonio_name,
        }
        ...
        if classification['tipo'] in ('EVITA', 'MIGLIORA'):
            self.capsule_memory.record(
                ...
                momentum=entry_momentum, volatility=entry_volatility,
                trend=entry_trend, matrimonio=matrimonio_name,
                action=classification['azione'], is_win=is_win,
                pnl=pnl_netto,
                note=f"{classification['sotto_tipo']} {classification['motivo'][:100]}",
            )

        # ── SALVA SU DB (per _carica_storia_dal_db al boot) ──────────────
        try:
            ...
            conn.execute("""
                CREATE TABLE IF NOT EXISTS trades (
                    ...
                    event_type TEXT, asset TEXT, price REAL,
                    size REAL, pnl REAL, direction TEXT, reason TEXT, data_json TEXT
                )
            """)
            conn.execute("""
                INSERT INTO trades
        ...
        self.heartbeat_data['narratore_trade_storia'] = _storia[-50:]
    ...
    def _carica_storia_dal_db(self):
        ...
        """Al boot carica gli ultimi 50 trade dal DB in narratore_trade_storia."""
        ...
        try:
            ...
            rows = conn.execute("""
                ...
                SELECT timestamp, direction, pnl, reason, data_json

                FROM trades WHERE event_type IN ('M2_EXIT','CLOSE','TRADE_CLOSE')
            ...
            for r in reversed(rows):
                ...
                storia.append({
                    ...
                    'pnl_lordo':  round(pnl + FEE_TRADE, 2),
            ...
            log.info(f"[V16] 📚 Caricati {len(storia)} trade da DB in narratore_trade_storia")
    ...
    def connect_binance(self):
        def on_message(ws, msg):
            try:
                ...
                if price > 0:
                    # Aggiorna heartbeat ad ogni tick — dashboard live
        ...
        def on_open(ws):
            log.info(f"[WS] ✅ Connesso a Binance {BINANCE_WS_URL}")
