...
class CapsuleManager:
    ...
    def valuta(self, contesto: dict) -> dict:
        ...
        for cap in self._cache:
            ...
            if atype in ("blocca_entry", "BLOCCA_ENTRY", "blocca_entry_oracle", "BLOCCA_CONTESTO", "blocca_contesto"):
                ...
                # MIGLIORA: abbassa la soglia retreat del PROFIT_LOCK
                # default 0.40 → es. 0.30 per catturare il picco prima
                retreat = act.get("params", {}).get("retreat", 0.30)
                try:
                    retreat = float(retreat)
                except (TypeError, ValueError):
                    retreat = 0.30
                if retreat < res["profit_lock_retreat"]:
                    res["profit_lock_retreat"] = retreat
                    res["reason"] = f"MIGLIORA_PROFIT_LOCK_RETREAT_{retreat:.2f}"
                self._fire(cap["id"], contesto)
                log.info(f"[CM] 🎯 PROFIT_LOCK_RETREAT={retreat:.2f} da {cap['id']}")
    ...
    def _fire(self, cap_id: str, ctx: dict):
        try:
            with sqlite3.connect(self.db_path) as c:
                c.execute("UPDATE capsule SET hits=hits+1, last_hit_ts=? WHERE id=?", (time.time(), cap_id,))
    ...
    def _pulisci_scadute(self):
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as c:
                ...
                if n:
                    log.info(f"[CM] 💤 {n} capsule mai usate in 30gg — disabilitate")
        except Exception as e:
