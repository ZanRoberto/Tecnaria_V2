...
<!-- PHANTOM TRACKER -->
  ...
  <div class="panel" style="margin-bottom:10px; border-color:#aa44ff; border-width:2px;">
    ...
    <div class="panel-body" style="padding:8px;">
      ...
      <div style="margin-top:8px;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px;">
        ...
        <!-- ════════ PASSO 12 — LENZE: dove pianto la lenza a 60s ════════ -->
        <div id="lenza-long-card" style="background:rgba(100,255,100,0.05);border:1px solid rgba(100,255,100,0.2);border-radius:6px;padding:8px;text-align:center;">
          <div style="font-size:9px;color:var(--dim)">🎣 LENZA LONG (+60s)</div>
          <div id="lenza-long" style="font-size:14px;font-weight:500;color:#639922;">—</div>
          <div id="lenza-long-tag" style="font-size:8px;color:var(--dim)">attesa</div>
        </div>
        <div id="lenza-short-card" style="background:rgba(255,100,100,0.05);border:1px solid rgba(255,100,100,0.2);border-radius:6px;padding:8px;text-align:center;">
          <div style="font-size:9px;color:var(--dim)">🎣 LENZA SHORT (-60s)</div>
          <div id="lenza-short" style="font-size:14px;font-weight:500;color:#E24B4A;">—</div>
          <div id="lenza-short-tag" style="font-size:8px;color:var(--dim)">attesa</div>
        </div>
      </div>
  ...
  <!-- ════════════════════════════════════════════════════════════════════ -->
  <!-- PASSO 15 — TATTICA DI PESCA (LibroPesca + storia visiva)              -->
  <!-- ════════════════════════════════════════════════════════════════════ -->
  <div class="panel" style="margin-bottom:10px; border-color:#1D9E75; border-width:2px;">
    <div class="panel-head" style="color:#1D9E75;">🎣 TATTICA DI PESCA — 5 orizzonti paralleli
      <span id="lp-status" style="font-size:9px; color:var(--dim)">—</span>
    </div>
    <div class="panel-body">
      <div style="font-size:9px; color:var(--dim); margin-bottom:8px;">
        Pianta 5 lenze (10/20/30/60/90s) quando il radar è acceso. Cattura → trade paper indipendente che chiude all'orizzonte. Il libro persiste tra restart.
      </div>

      <!-- 4 contatori principali -->
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:10px;">
        <div style="background:rgba(239,159,39,0.08);border:1px solid rgba(239,159,39,0.3);border-radius:6px;padding:8px;text-align:center;">
          <div style="font-size:9px;color:var(--dim);letter-spacing:0.5px;">PIANTATE</div>
          <div id="lp-piantate" style="font-size:18px;font-weight:500;color:#EF9F27;margin-top:2px;">—</div>
          <div id="lp-piantate-sub" style="font-size:8px;color:var(--dim);margin-top:2px;">in volo 0</div>
        </div>
        <div style="background:rgba(29,158,117,0.08);border:1px solid rgba(29,158,117,0.3);border-radius:6px;padding:8px;text-align:center;">
          <div style="font-size:9px;color:var(--dim);letter-spacing:0.5px;">CATTURATE</div>
          <div id="lp-catt" style="font-size:18px;font-weight:500;color:#1D9E75;margin-top:2px;">—</div>
          <div id="lp-catt-sub" style="font-size:8px;color:var(--dim);margin-top:2px;">0 vere · 0 barattoli</div>
        </div>
        <div style="background:rgba(136,135,128,0.08);border:1px solid rgba(136,135,128,0.3);border-radius:6px;padding:8px;text-align:center;">
          <div style="font-size:9px;color:var(--dim);letter-spacing:0.5px;">SCADUTE</div>
          <div id="lp-scad" style="font-size:18px;font-weight:500;color:#888780;margin-top:2px;">—</div>
          <div style="font-size:8px;color:var(--dim);margin-top:2px;">pesce non passato</div>
        </div>
        <div style="background:rgba(0,180,255,0.08);border:1px solid rgba(0,180,255,0.3);border-radius:6px;padding:8px;text-align:center;">
          <div style="font-size:9px;color:var(--dim);letter-spacing:0.5px;">PnL PAPER</div>
          <div id="lp-pnl" style="font-size:18px;font-weight:500;margin-top:2px;">—</div>
          <div style="font-size:8px;color:var(--dim);margin-top:2px;">cumulato pesca</div>
        </div>
      </div>

      <!-- Storia visiva: prezzo BTC + palle colorate (canvas puro) -->
      <div style="position:relative;width:100%;height:200px;background:rgba(0,0,0,0.2);border-radius:4px;margin-bottom:8px;">
        <canvas id="lp-storia" width="1200" height="200" style="width:100%;height:200px;"></canvas>
      </div>

      <!-- Legenda colori -->
      <div style="display:flex;flex-wrap:wrap;gap:10px;font-size:9px;color:var(--dim);margin-bottom:10px;justify-content:center;">
        <span style="display:flex;align-items:center;gap:4px;"><span style="width:9px;height:9px;background:#185FA5;display:inline-block;border-radius:50%;"></span>BTC</span>
        <span style="display:flex;align-items:center;gap:4px;"><span style="width:9px;height:9px;background:#EF9F27;display:inline-block;border-radius:50%;"></span>esca in attesa</span>
        <span style="display:flex;align-items:center;gap:4px;"><span style="width:9px;height:9px;background:#378ADD;display:inline-block;border-radius:50%;"></span>catturata</span>
        <span style="display:flex;align-items:center;gap:4px;"><span style="width:9px;height:9px;background:#1D9E75;display:inline-block;border-radius:50%;"></span>pesce vero</span>
        <span style="display:flex;align-items:center;gap:4px;"><span style="width:9px;height:9px;background:#E24B4A;display:inline-block;border-radius:50%;"></span>barattolo</span>
        <span style="display:flex;align-items:center;gap:4px;"><span style="width:9px;height:9px;background:#888780;display:inline-block;transform:rotate(45deg);"></span>scaduta</span>
      </div>

      <!-- PnL cumulativo coi dentini -->
      <div style="position:relative;width:100%;height:80px;background:rgba(0,0,0,0.2);border-radius:4px;margin-bottom:10px;">
        <canvas id="lp-pnl-chart" width="1200" height="80" style="width:100%;height:80px;"></canvas>
      </div>

      <!-- 5 orizzonti -->
      <div style="font-size:9px;color:var(--dim);letter-spacing:0.5px;margin-bottom:4px;">PERFORMANCE PER ORIZZONTE — chi vince?</div>
      <div id="lp-orizzonti" style="display:grid;grid-template-columns:repeat(5,1fr);gap:6px;margin-bottom:10px;">
        <!-- popolato dal JS -->
      </div>

      <!-- Lista lenze attive con countdown -->
      <div style="font-size:9px;color:var(--dim);letter-spacing:0.5px;margin-bottom:4px;">LENZE IN VOLO ADESSO</div>
      <div id="lp-attive" style="display:grid;grid-template-columns:repeat(2,1fr);gap:6px;max-height:240px;overflow-y:auto;">
        <div style="font-size:10px;color:var(--dim);grid-column:1/-1;text-align:center;padding:10px;">In attesa di radar acceso...</div>
      </div>
    </div>
  </div>
...
const SCPanel = (() => {
  ...
  function update(hb) {
    ...
    // ════════ PASSO 12 — Aggiorna LENZE (dove pianto la lenza a 60s) ════════
    const lzLong  = hb.lenza_long;
    const lzShort = hb.lenza_short;
    const lzAttiva= hb.lenza_attiva || 'NONE';
    const lzDelta = hb.lenza_delta  || 0;
    if (lzLong !== undefined && lzShort !== undefined) {
      const llEl = document.getElementById('lenza-long');
      const lsEl = document.getElementById('lenza-short');
      const llTag= document.getElementById('lenza-long-tag');
      const lsTag= document.getElementById('lenza-short-tag');
      const llCard= document.getElementById('lenza-long-card');
      const lsCard= document.getElementById('lenza-short-card');
      if (llEl) llEl.textContent  = '$' + (lzLong||0).toFixed(0);
      if (lsEl) lsEl.textContent  = '$' + (lzShort||0).toFixed(0);
      // Tag: indica se la lenza è ATTIVA (FUOCO acceso + predizione certificata)
      // o se è in attesa
      if (llTag) {
        if (lzAttiva === 'LONG') {
          llTag.textContent = '🔥 ATTIVA — pesce in arrivo';
          llTag.style.color = '#639922';
          if (llCard) { llCard.style.borderColor = '#639922'; llCard.style.background = 'rgba(100,255,100,0.12)'; }
        } else {
          llTag.textContent = 'delta ±$' + lzDelta.toFixed(0);
          llTag.style.color = 'var(--dim)';
          if (llCard) { llCard.style.borderColor = 'rgba(100,255,100,0.2)'; llCard.style.background = 'rgba(100,255,100,0.05)'; }
        }
      }
      if (lsTag) {
        if (lzAttiva === 'SHORT') {
          lsTag.textContent = '🔥 ATTIVA — pesce in arrivo';
          lsTag.style.color = '#E24B4A';
          if (lsCard) { lsCard.style.borderColor = '#E24B4A'; lsCard.style.background = 'rgba(255,100,100,0.12)'; }
        } else {
          lsTag.textContent = 'delta ±$' + lzDelta.toFixed(0);
          lsTag.style.color = 'var(--dim)';
          if (lsCard) { lsCard.style.borderColor = 'rgba(255,100,100,0.2)'; lsCard.style.background = 'rgba(255,100,100,0.05)'; }
        }
      }
    }
  ...
  // ════════════════════════════════════════════════════════════════════
  // PASSO 15 — TATTICA DI PESCA: aggiornamento UI da heartbeat
  // ════════════════════════════════════════════════════════════════════
  const LP_PRICE_BUF = [];   // ultimi 240 prezzi BTC per il grafico
  const LP_PRICE_MAX = 240;
  let LP_LAST_EVENTI = [];   // ultimi eventi conclusi (da hb.lp_eventi_recenti)
  let LP_LAST_ACTIVE = [];   // lenze in volo (da hb.lp_active)

  function updateLibroPesca(hb) {
    const statusEl = document.getElementById('lp-status');
    if (!statusEl) return;  // pannello non in DOM (vecchia versione)

    // 1) Banner stato
    if (hb.lp_enabled === false) {
      statusEl.textContent = '⏸ disattivo (env LIBRO_PESCA_ENABLED=false)';
      statusEl.style.color = '#888780';
    } else if (hb.lp_enabled === true) {
      statusEl.textContent = '● attivo · lavora in silenzio sopra il radar';
      statusEl.style.color = '#1D9E75';
    }

    // 2) Buffer prezzo per il grafico storico
    if (typeof hb.last_price === 'number') {
      LP_PRICE_BUF.push({ ts: Date.now()/1000, p: hb.last_price });
      while (LP_PRICE_BUF.length > LP_PRICE_MAX) LP_PRICE_BUF.shift();
    }

    // 3) Contatori principali
    const totale = hb.lp_totale || 0;
    const vere = hb.lp_vere || 0;
    const bar  = hb.lp_barattoli || 0;
    const scad = hb.lp_scadute || 0;
    const inVolo = hb.lp_in_volo || 0;
    const pnlTot = hb.lp_pnl_totale || 0;

    const elP = document.getElementById('lp-piantate');
    if (elP) elP.textContent = totale;
    const elPSub = document.getElementById('lp-piantate-sub');
    if (elPSub) elPSub.textContent = 'in volo ' + inVolo;

    const elC = document.getElementById('lp-catt');
    if (elC) elC.textContent = (vere + bar);
    const elCSub = document.getElementById('lp-catt-sub');
    if (elCSub) elCSub.textContent = vere + ' vere · ' + bar + ' barattoli';

    const elS = document.getElementById('lp-scad');
    if (elS) elS.textContent = scad;

    const elPnL = document.getElementById('lp-pnl');
    if (elPnL) {
      const sign = pnlTot >= 0 ? '+' : '';
      elPnL.textContent = sign + '$' + pnlTot.toFixed(2);
      elPnL.style.color = pnlTot >= 0 ? '#1D9E75' : '#E24B4A';
    }

    // 4) Eventi storici e lenze attive
    if (Array.isArray(hb.lp_eventi_recenti)) LP_LAST_EVENTI = hb.lp_eventi_recenti;
    if (Array.isArray(hb.lp_active))         LP_LAST_ACTIVE = hb.lp_active;

    // 5) Orizzonti
    const oz = hb.lp_orizzonti || {};
    const elOz = document.getElementById('lp-orizzonti');
    if (elOz) {
      const orizs = [10, 20, 30, 60, 90];
      let bestPct = -1;
      let bestH = -1;
      // trovo il vincente (richiede almeno 10 eventi)
      for (const h of orizs) {
        const o = oz[String(h)];
        if (!o) continue;
        const tot = (o.vere||0) + (o.barattoli||0) + (o.scadute||0);
        if (tot < 10) continue;
        if (o.pct_vincenti != null && o.pct_vincenti > bestPct) {
          bestPct = o.pct_vincenti;
          bestH = h;
        }
      }
      elOz.innerHTML = orizs.map(h => {
        const o = oz[String(h)] || {vere:0, barattoli:0, scadute:0, in_volo:0, pct_vincenti:null, pnl_totale:0};
        const tot = (o.vere||0) + (o.barattoli||0) + (o.scadute||0);
        const pct = o.pct_vincenti != null ? o.pct_vincenti.toFixed(0)+'%' : '—';
        const pctColor = o.pct_vincenti == null ? 'var(--dim)' :
                         o.pct_vincenti >= 55 ? '#1D9E75' :
                         o.pct_vincenti >= 45 ? '#EF9F27' : '#E24B4A';
        const pnlSign = (o.pnl_totale||0) >= 0 ? '+' : '';
        const winner = (h === bestH);
        return `<div style="background:${winner ? 'rgba(29,158,117,0.15)' : 'var(--bg2,rgba(255,255,255,0.03))'};border:1px solid ${winner ? '#1D9E75' : 'var(--border,rgba(255,255,255,0.1))'};border-radius:4px;padding:8px 4px;text-align:center;">
          <div style="font-size:10px;color:var(--dim);font-weight:500;">${h}s ${winner?' 🏆':''}</div>
          <div style="font-size:14px;font-weight:500;color:${pctColor};">${pct}</div>
          <div style="font-size:8px;color:var(--dim);line-height:1.4;margin-top:2px;">
            ${o.vere}V / ${o.barattoli}B / ${o.scadute}S<br>
            in volo ${o.in_volo}<br>
            PnL ${pnlSign}$${(o.pnl_totale||0).toFixed(1)}
          </div>
        </div>`;
      }).join('');
    }
    ...
    // 6) Lenze in volo
    const elAtt = document.getElementById('lp-attive');
    if (elAtt && Array.isArray(LP_LAST_ACTIVE)) {
      if (LP_LAST_ACTIVE.length === 0) {
        elAtt.innerHTML = `<div style="font-size:10px;color:var(--dim);grid-column:1/-1;text-align:center;padding:10px;">In attesa di radar acceso...</div>`;
      } else {
        const now = Date.now()/1000;
        elAtt.innerHTML = LP_LAST_ACTIVE.slice(0,20).map(L => {
          const tDallaPiantata = now - L.ts_piantata;
          const tRimasto = L.orizzonte_s - tDallaPiantata;
          const isCatt = L.stato === 'CATTURATA';
          const bgCol  = isCatt ? 'rgba(55,138,221,0.10)' : 'rgba(239,159,39,0.08)';
          const brCol  = isCatt ? '#378ADD' : 'rgba(239,159,39,0.4)';
          const stato  = isCatt ? '🔵 CATTURATA — trade in volo' : '🟡 esca in attesa';
          const dirCol = L.direzione === 'LONG' ? '#1D9E75' : '#E24B4A';
          return `<div style="background:${bgCol};border:1px solid ${brCol};border-radius:6px;padding:8px;">
            <div style="display:flex;justify-content:space-between;font-size:9px;margin-bottom:3px;">
              <span style="color:${dirCol};font-weight:500;">${L.direzione} · ${L.orizzonte_s}s</span>
              <span style="color:var(--dim);">${tRimasto > 0 ? tRimasto.toFixed(0)+'s rimasti' : 'in chiusura'}</span>
            </div>
            <div style="font-size:13px;font-weight:500;">$${L.prezzo_lenza.toFixed(0)}${isCatt ? ' → $'+L.prezzo_cattura.toFixed(0) : ''}</div>
            <div style="display:flex;justify-content:space-between;font-size:8px;color:var(--dim);margin-top:3px;">
              <span>lasco ±$${L.lasco.toFixed(1)}</span>
              <span>${stato}</span>
            </div>
          </div>`;
        }).join('');
    ...
    // 7) Disegna i due canvas (storia + PnL)
    drawLibroPescaCanvas();
  }

  function drawLibroPescaCanvas() {
    // Storia: prezzo BTC + palle colorate degli eventi
    const cv = document.getElementById('lp-storia');
    if (!cv) return;
    ...
    if (LP_PRICE_BUF.length < 2) {
      ctx.fillStyle = '#666';
      ctx.font = '10px monospace';
      ctx.textAlign = 'center';
      ctx.fillText('In attesa di prezzi…', cssW/2, cssH/2);
      return;
    }

    // Range Y: include sia prezzi sia prezzi delle lenze recenti
    const allP = LP_PRICE_BUF.map(x => x.p);
    LP_LAST_EVENTI.forEach(e => { if (e.prezzo_lenza) allP.push(e.prezzo_lenza); if (e.prezzo_chiusura) allP.push(e.prezzo_chiusura); });
    LP_LAST_ACTIVE.forEach(L => { if (L.prezzo_lenza) allP.push(L.prezzo_lenza); });
    const pMin = Math.min(...allP), pMax = Math.max(...allP);
    const pad = Math.max((pMax-pMin)*0.15, 5);
    const yMin = pMin - pad, yMax = pMax + pad;
    const yMap = p => cssH - ((p - yMin)/(yMax - yMin)) * (cssH - 20) - 10;

    // Range X: timeline da primo prezzo nel buffer
    const tFirst = LP_PRICE_BUF[0].ts;
    const tLast  = LP_PRICE_BUF[LP_PRICE_BUF.length-1].ts;
    const tRange = Math.max(tLast - tFirst, 60);
    const xMap = ts => ((ts - tFirst)/tRange) * (cssW - 20) + 10;

    // Linea prezzo BTC
    ctx.strokeStyle = '#378ADD';
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    LP_PRICE_BUF.forEach((pt, i) => {
      const x = xMap(pt.ts), y = yMap(pt.p);
      if (i === 0) ctx.moveTo(x,y); else ctx.lineTo(x,y);
    });
    ctx.stroke();

    // Palline eventi storici (catture verdi/rosse + scadute grigie)
    LP_LAST_EVENTI.forEach(e => {
      if (e.ts_piantata < tFirst) return;
      // pallina alla cattura/scadenza
      const tEvent = e.ts_chiusura || e.ts_cattura || e.ts_piantata;
      const pEvent = e.prezzo_chiusura || e.prezzo_cattura || e.prezzo_lenza;
      if (tEvent < tFirst || tEvent > tLast + 5) return;
      let color = '#888780';
      let shape = 'circle';
      if (e.esito_finale === 'VERA')      color = '#1D9E75';
      else if (e.esito_finale === 'BARATTOLO') color = '#E24B4A';
      else if (e.esito_finale === 'SCADUTA')   { color = '#888780'; shape = 'square'; }
      const x = xMap(tEvent), y = yMap(pEvent);
      ctx.fillStyle = color;
      ctx.strokeStyle = '#fff';
      ctx.lineWidth = 1.5;
      if (shape === 'square') {
    ...
    // Palline lenze ATTIVE (giallo attesa, blu catturata)
    LP_LAST_ACTIVE.forEach(L => {
      if (L.ts_piantata < tFirst) return;
      const isCatt = L.stato === 'CATTURATA';
      const color = isCatt ? '#378ADD' : '#EF9F27';
      const tShow = isCatt ? (L.ts_cattura || L.ts_piantata) : L.ts_piantata;
      const pShow = isCatt ? (L.prezzo_cattura || L.prezzo_lenza) : L.prezzo_lenza;
      const x = xMap(tShow), y = yMap(pShow);
      ctx.fillStyle = color;
      ctx.strokeStyle = '#fff';
      ctx.lineWidth = 1.5;
    ...
    // Asse Y minimal: prezzo min/max
    ctx.fillStyle = '#888780';
    ctx.font = '9px monospace';
    ctx.textAlign = 'left';
    ctx.fillText('$' + pMax.toFixed(0), 4, 12);
    ctx.fillText('$' + pMin.toFixed(0), 4, cssH-4);

    // PnL cumulativo (dentini)
    drawLibroPescaPnL();
  }

  function drawLibroPescaPnL() {
    const cv = document.getElementById('lp-pnl-chart');
    if (!cv) return;
    const dpr = window.devicePixelRatio || 1;
    const cssW = cv.clientWidth, cssH = cv.clientHeight;
    if (cv.width !== cssW * dpr || cv.height !== cssH * dpr) {
    ...
    // Ricostruisco PnL cumulativo dagli eventi ordinati per ts_chiusura
    const eventi = LP_LAST_EVENTI
      .filter(e => e.esito_finale === 'VERA' || e.esito_finale === 'BARATTOLO')
      .filter(e => e.ts_chiusura != null)
      .sort((a,b) => a.ts_chiusura - b.ts_chiusura);

    if (eventi.length === 0) {
      ctx.fillStyle = '#666';
      ctx.font = '10px monospace';
      ctx.textAlign = 'center';
      ctx.fillText('In attesa di trade chiusi…', cssW/2, cssH/2);
      return;
    }

    let acc = 0;
    const pts = eventi.map(e => { acc += (e.pnl_paper||0); return { t: e.ts_chiusura, pnl: acc }; });
    const tFirst = pts[0].t, tLast = pts[pts.length-1].t;
    const pnlMin = Math.min(0, ...pts.map(p => p.pnl));
    const pnlMax = Math.max(0, ...pts.map(p => p.pnl));
    const pad = Math.max((pnlMax-pnlMin)*0.15, 1);
    const yMin = pnlMin - pad, yMax = pnlMax + pad;
    const xMap = t => ((t - tFirst)/Math.max(tLast-tFirst, 1)) * (cssW - 30) + 10;
    const yMap = v => cssH - ((v - yMin)/(yMax - yMin)) * (cssH - 20) - 10;

    // Linea zero
    const yZero = yMap(0);
    ctx.strokeStyle = 'rgba(136,135,128,0.3)';
    ctx.setLineDash([2,3]);
    ctx.beginPath(); ctx.moveTo(10,yZero); ctx.lineTo(cssW-20,yZero); ctx.stroke();
    ctx.setLineDash([]);

    // PnL stepped
    ctx.strokeStyle = acc >= 0 ? '#1D9E75' : '#E24B4A';
    ctx.fillStyle = acc >= 0 ? 'rgba(29,158,117,0.15)' : 'rgba(226,75,74,0.15)';
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.moveTo(xMap(tFirst), yMap(0));
    pts.forEach((p,i) => {
      const x = xMap(p.t), y = yMap(p.pnl);
      if (i === 0) ctx.lineTo(x, yMap(0));
      ctx.lineTo(x, y);
    });
    const xLast = xMap(tLast);
    ctx.lineTo(xLast, yZero);
    ctx.closePath();
    ctx.fill();
    ...
    ctx.font = '9px monospace';
    ctx.textAlign = 'right';
    const lastSign = acc >= 0 ? '+' : '';
    ctx.fillText(lastSign + '$' + acc.toFixed(2), cssW - 4, yMap(acc) - 4);
  }
