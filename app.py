...
class OvertopBassanoV15Production:
        ...
        if now - self.last_config_check > 30:
            if self.config_reloader.check_reload():
                if self.capsule_runtime.reload():
                    caps_attive = [c.get('id','?') for c in self.capsule_runtime.capsules if c.get('enabled')]  # FIX E03: chiave corretta CM
    ...
    def _evaluate_entry(self, price, momentum, volatility, trend):
        ...
        ctx_caps = {
            ...
            'volatility':       volatility,
            'trend':            trend,
            'direction':        self.campo._direction,  # FIX E02: abilita VETO_LONG/VETO_SHORT Motore 1
    ...
    def run(self):
        ...
        self.connect_binance()
        # FIX E05: avvia OracleAuto background worker — modalità controllata da ENV
        try:
            import oracle_auto
            oracle_auto.start_background(bot_instance=self)
            _oracle_mode = os.environ.get("ORACLE_AUTO_MODE", "MANUAL").upper()
            if _oracle_mode in ("AUTO", "MANUAL"):
                oracle_auto.set_mode(_oracle_mode)
            else:
                log.warning(f"[ORACLE_AUTO] ORACLE_AUTO_MODE non valido: {_oracle_mode} — uso MANUAL")
                oracle_auto.set_mode("MANUAL")
            log.info(f"[ORACLE_AUTO] Background worker avviato da run() mode={_oracle_mode}")
        except ImportError:
            log.warning("[ORACLE_AUTO] oracle_auto.py non trovato — worker non avviato")
        except Exception as _oa_e:
            log.error(f"[ORACLE_AUTO] errore avvio worker: {_oa_e}")
