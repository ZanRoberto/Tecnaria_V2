...
<!-- PHANTOM TRACKER -->
  ...
  <div class="panel" style="margin-bottom:10px; border-color:#00aaff; border-width:2px;">
    <div class="panel-head blue">ANALISI TRADE
...
function update() {
  fetch('/trading/status').then(r=>r.json()).then(d => {
    ...
    m2log.forEach(line => {
      const tsParts = line.split(' '); const tsStr = tsParts[0] || ''; const tsMatch = tsStr.length === 8 && tsStr[2] === ':' ? [tsStr, tsStr] : null;
      if (!tsMatch) return;
      const lineTs = new Date().toDateString() + ' ' + tsMatch[1];
      const ts = new Date(lineTs).getTime();
      if (line.includes('ENTRY')) {
        const dir = line.includes('SHORT') ? 'SHORT' : 'LONG';
        var scoreIdx = line.indexOf('score='); var scoreM = scoreIdx >= 0 ? [null, line.slice(scoreIdx+6).split(' ')[0].split('/')[0]] : null;
    ...
    if (analisiEl && analisiList.length > 0) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        html += '<div style="padding:8px;margin-bottom:6px;border-radius:4px;background:' + bg + ';border-left:3px solid ' + col + '">';
        html += '<div style="display:flex;justify-content:space-between">';
        html += '<span style="font-size:10px;font-weight:bold;color:' + col + '">' + (isWin ? 'WIN' : 'LOSS') + ' ' + (a.ts||'') + ' score=' + scoreStr + '</span>';
        html += '<span style="font-size:10px;color:' + col + '">' + pnlStr + '</span>';
        html += '</div>';
        html += '<div style="font-size:9px;color:var(--dim)">' + (a.motivo||'') + '</div>';
        html += '<div style="font-size:10px;color:var(--text)">' + testo + '</div>';
        html += '</div>';
      }
